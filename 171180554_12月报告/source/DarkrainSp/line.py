...
class Line(primitive.Primitive):
    ...
    def returnPointList(self):
        ...
        if self.startPos == self.endPos:
            self.pointList.append(self.startPos)
            return
    ...
    def clip(self, x1: int, y1: int, x2: int, y2: int, algorithm: str):
        ...
        if algorithm == "Cohen-Sutherland":
            self.clip_Cohen_Sutherland(xMin, yMin, xMax, yMax)
        else:
            self.clip_Liang_Barsky(xMin, yMin, xMax, yMax)
        self.rewrite()
    ...
    def clip_Liang_Barsky(self, x1: int, y1: int, x2: int, y2: int):
        # LOG("liang start")
        x0 = self.startPos.x()
        y0 = self.startPos.y()
        dx = self.endPos.x() - self.startPos.x()
        dy = self.endPos.y() - self.startPos.y()
        p = [-dx, dx, -dy, dy]
        q = [self.startPos.x() - x1, x2 - self.startPos.x(),
             self.startPos.y() - y1, y2 - self.startPos.y()]
        u1 = 0
        u2 = 1
        exist = 1
        ...
        if exist == 0:
            ...
            self.startPos = QtCore.QPoint(toInt(x0 + dx * u1), toInt(y0 + dy * u1))
            self.endPos = QtCore.QPoint(toInt(x0 + dx * u2), toInt(y0 + dy * u2))
        pass
